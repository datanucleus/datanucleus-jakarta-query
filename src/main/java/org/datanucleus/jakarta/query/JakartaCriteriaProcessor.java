...
Contributors:
   ...
**********************************************************************/
package org.datanucleus.jakarta.query;
...
import org.datanucleus.jakarta.query.AnnotationProcessorUtils.TypeCategory;

/**
 * Annotation processor for Jakarta Persistence to generate "dummy" classes for all Entity classes for use with Criteria API.
 * Any class ({MyClass}) that has a Jakarta Persistence "class" annotation will have a stub class ({MyClass}_) generated.
 * <ul>
 ...
 * <li>The metamodel class X_ must be annotated with the jakarta.persistence.StaticMetamodel annotation</li>
...
public class JakartaCriteriaProcessor extends AbstractProcessor
{
    ...
    {
        if (el == null || !isJakartaAnnotated(el))
        {
            return;
        }
        ...
        System.out.println("DataNucleus : Jakarta Persistence Criteria - " + className + " -> " + classNameNew);
        ...
        try
        {
            ...
            try
            {
                ...
                w.append("import jakarta.persistence.metamodel.*;\n");
                w.append("\n");
    ...
    public TypeElement getPersistentSupertype(TypeElement element)
    {
        ...
        if (isJakartaAnnotated(superElement))
        {
            return superElement;
        }
            ...
            if (AnnotationProcessorUtils.isMethod(member))
            {
                ...
                if (AnnotationProcessorUtils.isJavaBeanGetter(method) || AnnotationProcessorUtils.isJavaBeanSetter(method))
                {
                    ...
                    while (annIter.hasNext())
                    {
                        ...
                        if (annTypeName.startsWith("jakarta.persistence"))
                        {
     ...
     * @param el The class element
     * @return Whether it is to be considered a Jakarta Persistence annotated class
     */
